...
-- book/cafeitem: any signed-in staff too -- a sale must be able to decrement stock on checkout,
-- and a return must be able to put it back. The app UI still restricts add/edit/delete of the
-- catalog itself (title, price, ...) to owner (stock-adjust screens allow manager). This is a
...
-- cat/settings/customer: owner only.
create policy r_ins on records for insert with check(me() is not null and (kind in('order','exp','book','cafeitem') or me()='owner'));
create policy r_upd on records for update using(me() is not null and (kind in('order','exp','book','cafeitem') or me()='owner')) with check(me() is not null and (kind in('order','exp','book','cafeitem') or me()='owner'));
