...
1. Open the address in Chrome, sign in, then open **Settings**: confirm the business name (defaults to "OG Book Cafe"), address, phone, GSTIN, GST %, invoice prefix. Add your tables.
2. The **Menu** tab is pre-seeded on first sign-in with the OG Book Cafe menu (Hot Tea, Ice Tea, Shakes, Sandwich, Kulvich, Noodles, Specials) straight from the menu boards. Add, edit or remove items there any time. For sizes, type `Half:49, Full:69` in the sizes field.
3. Install the app: Chrome menu > **Install app** (phone: **Add to Home Screen**). **Sign in once while online**; after that it works offline.

## 5b. Order-placed alert sound
Every time **Send to kitchen** is tapped, the till plays a loud 3-tone alert immediately, and any other screen signed in (e.g. a kitchen tablet open on the **Kitchen** tab) plays the same alert within a few seconds once it syncs the new order. Tap the 🔔 button in the header any time to test the sound and to "unlock" audio on that device (browsers require one tap/click before they'll allow sound). Keep the device unmuted and its volume up.
